...
public class User {
	 ...
	 * 			*객체의 초기화 작업 - 객체의 멤버변수의 값을 초기화 하는 작업이다.
	 * 							- 객체의 멤버변수에 값을 대입하는 작업이다.
	 * 			*멤버메소드의 실행 전에 반드시 수행해야 하는 전처리 작업
	 * 		-new 생성자()와 같이 객체 생성될 때만 사용될 수 있는 메소드다.
	 * 			*new 키워드와만 사용 할 수 있다.
	 * 		-생성자 메소드 조건(객체 생성 직후에 바로 실행될 메소드의 조건)
	...
	// 생성자 중복정의 
	// User (int, String, String, String)값을 매개변수로 가지는 생성자 메소드 중복정의 함
