...
public class BookApp3 {
	...
	public static void main(String[] args) {
		...
		Event event2 = new Event("신규가입 할인","4월 두 번째 가입고객 할인","4/5","4/12",false);
		...
		//매개변수로 전달
		Book book2 = new Book(120,"자바의정석","남궁성","도우출판사",28000,false,0.15,event2);
		book2.setGift(new Gift("메롱",1000));
		
		//book3가 참조하는 Book객체에 정의된 setter메소드에 직접 생성한 Event객체와 Gift객체의 주소값을 
		//메소드의 매개변수로 전달
		Book book3 = new Book(130,"스프링인액션","심재철","한빛미디어",34000,false,0.1);
		
		book3.setEvent(new Event("특가할인","추가구매 고객대상","4/1","4/15",false));
		book3.setGift(new Gift("별책부록",3000));
		
		//books 참조변수가 참조하는 배열객체의 각각의 칸에 순서대로 위에서 생성한 Book객체의 
		books[0] = book1;
		books[1] = book2;
		books[2] = book3;
		
		//books배열에 저장된 모든 Book객체의 책정보, 해당 이벤트 정보, 사은품 정보 출력하기
		for (Book book : books) {
